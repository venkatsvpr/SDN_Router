...
void main_loop()
{
 	...
 	while(TRUE)
	{
		...
		{
//			printf ("Setting data_control_sock on master list <%d>\r\n",data_control_socket);
	...
	t_timer.tv_usec = p_timer.tv_usec;
        ...
        for(sock_index=0; sock_index<=head_fd; sock_index+=1){
            ...
            if(FD_ISSET(sock_index, &watch_list)){
				...
				//	printf (" Setting <%d> on master_list \r\n",fdaccept);
				...
				//	printf (" <%s:%d> accepting <%d>\r\n",__func__,__LINE__,data_socket);
				}
                ...
                else if(sock_index == router_socket)
				{
					...
					//printf ("<%s:%d> router socket select \r\n",__func__,__LINE__);
				...
				//	printf ("<%s:%d> data socket selecct \r\n",__func__,__LINE__);
				}
		...
		for (int j=0; j < g_num_of_routers; j++)
	 	{
			...
			/*chekc this venkat */
///			if (g_peers[j].router_id == g_peers[i].router_id)	
//			cost = 65535;
			cost1 = htons(cost);	
...
//		printf ("Send success <%s:%d> sent:%d port%d routersocket:%d  ",__func__,__LINE__,sent_byte,htons(router_port),router_socket);
//		print_ip(g_peers[i].router_ip);
	}
...
//	printf ("<num of update:%d port:%d from peer_router_id %d \r\n",num_of_update,src_router_port,peer_router_id);
...
//		get_router_id(ip, port, &router_id);
...
int
Cost_Reach (uint16_t router_id, uint16_t *cost, uint16_t *peer)
{
	int status=0;
...
int
Cost_to_Reach (uint16_t router_id, uint16_t *cost, uint16_t *peer)
{
    for (int i = 0; i<g_num_of_routers; i++)
    {
        if (g_routing_table[i].router_id == router_id)
...
int
find_index_in_rtable (uint16_t router_id)
{
	for (int i = 0 ;i <g_num_of_routers; i++)
	{
		if (g_routing_table[i].router_id == router_id)
		{
//			printf ("<%s: routre_id :%d index :%d\r\n",__func__,router_id,i);
			return i;
		}
	}
	return -1;	
...
int
find_index_with_ip (uint32_t router_ip)
{
	for (int i = 0 ;  i<g_num_of_routers; i++)
	{
...
bool
is_inactive_peer (uint16_t router_id)
{
	int index = -1;
	index = find_index_in_rtable (router_id);
	if (index<0)
	{
		//printf (" Something fundamentaly wrong.. panic..<%s:%d>  router_id:%d iindex:%d g_in:%d\r\n",__func__,__LINE__,router_id,index,g_interval);
		return FALSE;
	}		
...
void
Distance_Vector_Update (struct rtable_entry PeerUpdate[5])
{
	for (int i = 0; i < g_num_of_routers; i++)
	{
...
/*
		...
		if (g_peers[index].cost != 65535)
		{
			if (g_peers[index].cost <= PeerUpdate[i].cost)
			{
				g_routing_table[index].cost = g_peers[index].cost;
				g_routing_table[index].next_hop_id =g_peers[index].router_id;
				continue;	
			}
		}
*/
...
void update_rtable_inactive(uint16_t router_id)
{
	for (int i =0; i<g_num_of_routers; i++)
	{
		...
		{	
			/* more code for changning the nexthopid may be required */
...
int route_packets(int sock_index)
{
	...
	{
            free(payload);
//			printf ("<%s:%d> something fundamentally wrong \r\n",__func__,__LINE__);	
            return FALSE;
    }
...
//	printf ("Received router_ ip is ");
//	print_ip(router_ip);
//	printf ("my router ip is ");
//	print_ip(my_router_ip);
//	printf ("%s\r\n",payload+12);
	...
	if (my_router_ip == router_ip)
	{
		...
		int file_fd = 0;
		int retval = fwrite(payload+12, 1, 1024, file_fp);	
//		printf ("wrote %d bytes at fd %d \r\n",retval,file_fd);
	//	fclose(fp);
	}
	else
	{
//		printf ("sending to next_hop \r\n");
		memcpy (&ttl, payload+5, 1);
		uint16_t seq =0;
		memcpy (&seq, payload+6, 2);
		ttl = ttl -1;
//		printf ("ttl is %d seq %d\r\n",ttl,ntohs(seq));
		if (ttl == 0)
		{
//			printf (" hop coount reached \r\n");
			goto free;
		}
...
void 
Send_to_Next_Hop (uint32_t router_ip, char *payload)
{
